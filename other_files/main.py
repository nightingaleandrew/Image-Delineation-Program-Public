...
class PageOne(tk.Frame):
    def __init__(self, parent, controller):
        ...
        for i in range(self.num_notebooks):
            name = "load a stack (tab " + str(i) +")"
            state = "normal"
            if i > 0:
                state = "disabled"
            self.load_stacks_buttons_dict.append({"name": name, "command":lambda notebook = self.notebook_frames[i]['notebook']: self.load_scans(notebook), "default_state": state, "side":"left", "width":25})
        ##**NEW**##

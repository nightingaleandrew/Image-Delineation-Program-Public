...
class jsonFileReaderWriter:
    ...
    def add_record(self, dict):
        try:
            ...
            data = self.read_file()
            data[0][self.datakey].append(dict)
            self.create_json_file(data)
        except:
    ...
    def read_file(self):
        try:
            if os.path.exists(self.filename):
                ...
                with open(self.filename) as f:
                    currentdata = json.load(f)
                    print("CURRENTDETA", currentdata)
                    if self.datakey not in currentdata[0]:
                        currentdata[0][self.datakey] = []
                    return currentdata
            else:
    ...
    def remove_record(self, data, id):
        try:
            if os.path.exists(self.filename):
                data[0][self.datakey] = [item for item in data[0][self.datakey] if item['id'] != id] #remove the polygon with the same id
